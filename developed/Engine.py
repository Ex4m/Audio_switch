...
class SetupStuff:
    ...
    def get_list(self, display_output=True):
        self.result = sb.run(
            ["powershell", "Get-AudioDevice -list"], capture_output=True, text=True)
    ...
    def get_device_key(self, value, index):
        ...
        if self.device_dict is None:
            raise TypeError(
                "origin dictionary is not created, try use .get_list ->.convert_list first")
    ...
    def add_device(self, *index):
        ...
        for i in index:
            ...
            if len(self.selected_list) < 2:
                ...
                raise ValueError(
                    f"{self.get_device_key('Name',i)} cannot be added. Your list of selected items is full")
    ...
    def is_module_installed(self):
        ...
        """
        output = sb.run(["powershell", "Get-InstalledModule"],
    ...
    def pws_command(self, command, admin=False):
        if admin == True:
            result = sb.run(["runas", "/user:Administrator"] +
...
Unrestricted: Všechny skripty mohou být spuštěny bez omezení."""
    ...
    def set_script_policy(self):
        ...
        command = ["powershell",
                   f"Set-ExecutionPolicy {options} -Scope CurrentUser -Force"]
    ...
    def install_preq(self):
        ...
        if self.check is None:
            raise ValueError(
                "First you need to check if the module is installed or not, run 'is_module_installed'")
    ...
    def uninstall_preq(self):
        inp = input(
            "Do you want to uninstall Powershell AudioDevice Module (y/n): ")
        if self._confirm(inp):
            try:
                command = ["powershell",
                           "Uninstall-Module -Name AudioDeviceCmdlets -Force"]
                self.pws_command(command)
    ...
    while True:
        ...
        match choice:
            ...
            case "8":
                ...
                hotkey_sel = input(
                    "Select one of the above mentioned types (i.e.TRIGGER) for the hotkey you wish to edit: ").upper()
                try:
